...
def text2image_ldm_stable(
    ...
    print("Context: ", context.shape, context.requires_grad)
    latent, latents = init_latent(latent, model, height, width, generator, batch_size)
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        ...
        def forward(x, context=None, mask=None):
            ...
            # print("attn:", attn.requires_grad, attn.grad_fn, attn.shape)
            attn = controller(attn, is_cross, place_in_unet)
            # print("control attn:", attn.requires_grad, attn.grad_fn, attn.shape)
    ...
    for net in sub_nets:
        if "down" in net[0]:
            cross_att_count = cross_att_count + register_recr(net[1], 0, "down")
        elif "up" in net[0]:
            cross_att_count = cross_att_count + register_recr(net[1], 0, "up")
        elif "mid" in net[0]:
            cross_att_count = cross_att_count + register_recr(net[1], 0, "mid")
...
def get_word_inds(text: str, word_place: int, tokenizer):
    ...
    if len(word_place) > 0:
        ...
        for i in range(len(words_encode)):
            cur_len = cur_len + len(words_encode[i])
